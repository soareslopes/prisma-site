...
export default function InsightDetail() {
  ...
  return (
    <main>
      {/* Header */}
      <div className="py-20 lg:py-28" style={{ backgroundColor: '#0A0F1E' }}>
        <div className="w80">
          <Link to="/insights" className="inline-flex items-center gap-2 text-[12px] font-medium tracking-[0.06em] uppercase mb-12 hover:opacity-70" style={{ color: 'rgba(255,255,255,0.4)' }}>
          ...
          <p className="text-[15px]" style={{ color: 'rgba(255,255,255,0.45)' }}>By {insight.author}</p>
        </div>
      </div>

      {/* Article body */}
      <div className="w80 py-16 lg:py-24">
        ...
        <div className="max-w-3xl space-y-7">
          {insight.content
            ...
            : [
                ...
                'Our analysis integrates multiple data streams to build a comprehensive picture of the phenomenon under study. This multi-source approach is methodologically essential: individual data sources contain systematic biases and coverage gaps that can only be addressed through triangulation.',
                'The implications for organizational practice are significant. Decision-makers who rely on non-spatial analytical frameworks will systematically underestimate the heterogeneity of outcomes across territories, misallocate resources and miss early-warning signals only visible in the spatial distribution of phenomena.',
                "Evidence-based territorial governance requires not only the availability of analytical capacity, but the institutional structures to translate that capacity into decision-relevant intelligence. This translation challenge is the core of Prisma's practice.",
              ].map((para, i) => (
                <p key={i} className="text-[16px] leading-[1.9]" style={{ color: '#444444' }}>{para}</p>
              ))
          }
        ...
        <div className="mt-16 pt-10 border-t flex items-start gap-5" style={{ borderColor: '#E5E5E5' }}>
          <div className="w-11 h-11 flex items-center justify-center flex-shrink-0 text-[14px] font-semibold" style={{ backgroundColor: '#EEF4FF', color: '#2E7CF6' }}>P</div>
          <div>
            <p className="text-[14px] font-semibold mb-1" style={{ color: '#111111' }}>{insight.author}</p>
            <p className="text-[13px]" style={{ color: '#888888' }}>Prisma · Spatial Intelligence & Territorial Analytics · Lisboa, Portugal</p>
          </div>
        </div>
      </div>
      ...
      <div className="py-16 lg:py-20" style={{ backgroundColor: '#F7F7F7', borderTop: '1px solid #E5E5E5' }}>
        <div className="w80">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-14">
            {others.map((item) => (
              <Link key={item.id} to={`/insights/${item.id}`} className="group block border-t py-7" style={{ borderColor: '#D0D0D0' }}>
                ...
                <h3 className="text-[16px] font-semibold leading-snug mb-3 transition-colors duration-200 group-hover:text-[#2E7CF6]" style={{ color: '#111111' }}>{item.title}</h3>
